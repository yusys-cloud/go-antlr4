...
func (s *CompactConstructorDeclarationContext) GetConstructorBody() IBlockContext { return s.constructorBody }


func (s *CompactConstructorDeclarationContext) SetConstructorBody(v IBlockContext) { s.constructorBody = v }
		...
		if p.GetInterpreter().AdaptivePredict(p.BaseParser, p.GetTokenStream(), 89, p.GetParserRuleContext()) == 1 {
			{
				...
				goto errorExit} else if p.GetInterpreter().AdaptivePredict(p.BaseParser, p.GetTokenStream(), 89, p.GetParserRuleContext()) == 2 {
			{
				p.SetState(832)
				p.ElementValue()
			}
...
func (p *JavaParser) Sempred(localctx antlr.RuleContext, ruleIndex, predIndex int) bool {
	...
	case 107:
			...
			if localctx != nil { t = localctx.(*GuardedPatternContext) }
